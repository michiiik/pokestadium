...
#ifdef NON_MATCHING
f32 Math_PointToLineProjection(
...
#else
#pragma GLOBAL_ASM("asm/us/nonmatchings/33FE0/Math_PointToLineProjection.s")
#endif

#ifdef NON_MATCHING
f32 Math_ClosestPointOnLine(
    f32 px, f32 py, f32 pz,
    f32 ax, f32 ay, f32 az,
    f32 bx, f32 by, f32 bz,
...
#else
#pragma GLOBAL_ASM("asm/us/nonmatchings/33FE0/Math_ClosestPointOnLine.s")
#endif
...
void Model_BuildVertexRuntimeData(StadiumModel* model, MtxF* mtx) {
    ...
    for(var_s3 = 0; var_s3 < 0x10; var_s3++) {
        ...
        for(var_s0 = 0; var_s0 < segment->vertexCount; var_s0++) {
            if (var_s3 == (Model_GetVertexJoint(temp_s2, var_s0) & 0xFFFF)) {
    ...
    mvtx = base;
    var_a1 = 0;
    indexTable = Memmap_GetSegmentVaddr(segment->childIndexSegment);
...
#else
void Model_BuildVertexRuntimeData(StadiumModel*, MtxF* mtx);
#pragma GLOBAL_ASM("asm/us/nonmatchings/33FE0/Model_BuildVertexRuntimeData.s")
#endif
...
#ifdef NON_MATCHING
void Model_LinkNearestVertices(StadiumModel* model) {
    ...
    for(i = 0; i < segment->vertexCount; i++) {
        ...
        if (temp_v1 == i) {
            var_fv0 = D_8007C5B8;
            var_a0 = var_a2;
            for(j = 0; j < model->transformCount; j++) {
                if (var_a0->linkDistance < var_fv0) {
                    var_fv0 = var_a0->linkDistance;
                    var_a2->nextIndex = j;
                }
                var_a0 += 1;
            }
...
#else
#pragma GLOBAL_ASM("asm/us/nonmatchings/33FE0/Model_LinkNearestVertices.s")
#endif
...
#ifdef NON_MATCHING
void Model_UpdateRemappedVertices(ModelSegment* segment, MtxF* mtx, ModelVertex* mvtx) {
    Vec3sPad* var_s2;
    ModelVertex* tmp;
    s16* var_s7;
    s32 i;
...
#else
void Model_UpdateRemappedVertices(ModelSegment*, MtxF*, ModelVertex*);
#pragma GLOBAL_ASM("asm/us/nonmatchings/33FE0/Model_UpdateRemappedVertices.s")
#endif

void Model_ComputeScaledDistance(Vec3f* a, Vec3f* b, Vec3f* scale) {
    f32 dx;
    f32 dy;
    f32 dz;
...
#ifdef NON_MATCHING
void Model_ExtrapolateVertexPosition(Vec3f* from, Vec3f* to, f32 currentTime, f32 deltaTime) {
    f32 sp14;
    f32 temp_ft4;
    f32 temp_fv1;
    f32 scale;
...
#else
void Model_ExtrapolateVertexPosition(Vec3f*, Vec3f*, f32, f32);
#pragma GLOBAL_ASM("asm/us/nonmatchings/33FE0/Model_ExtrapolateVertexPosition.s")
#endif

#ifdef NON_MATCHING
void Model_SpringBlendPosition(PosBlend* src, PosBlend* dst, f32 totalTime, f32 elapsed, f32 stiffness) {
    f32 sp3C;
    f32 sp38;
    f32 sp34;
    f32 sp20;
...
#else
void Model_SpringBlendPosition(PosBlend*, PosBlend*, f32, f32, f32);
#pragma GLOBAL_ASM("asm/us/nonmatchings/33FE0/Model_SpringBlendPosition.s")
#endif
...
void Model_ApplyTransformCommands(ModelSegment* segment, ModelVertex* vertices, StadiumModel* model, f32 deltaTime) {
    ...
    while(1) {
        ...
        if ((var_a0 != 0) || (var_a2 != 0)) {
            ...
            Model_ComputeScaledDistance(temp_s0, temp_s1, &model->position);
            Model_ExtrapolateVertexPosition(temp_s0, temp_s1, temp_fs0, deltaTime);
        }
    }
}
#else
#pragma GLOBAL_ASM("asm/us/nonmatchings/33FE0/Model_ApplyTransformCommands.s")
#endif
...
#ifdef NON_MATCHING
void Model_OffsetVertexHeightsByClass(ModelSegment* segment, ModelVertex* vertices, f32 yOffset) {
    s16* indexTable;
    s16* var_s2;
    s32 temp_v0;
    s32 i;
...
#else
void Model_OffsetVertexHeightsByClass(ModelSegment*, ModelVertex*, f32);
#pragma GLOBAL_ASM("asm/us/nonmatchings/33FE0/Model_OffsetVertexHeightsByClass.s")
#endif
