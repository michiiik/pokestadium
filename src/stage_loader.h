...
typedef struct unk_D_800A7450 {
    /* 0x00 */ u8 unk_00; // saved copy of the active context's unk_10
    /* 0x01 */ u8 isHighRes; // saved copy of the active context's isHighRes
    /* 0x02 */ u8 unk_02; // saved copy of the active context's unk_0D
    /* 0x03 */ u8 fadeMode; // saved copy of the active context's fadeMode
    /* 0x04 */ Gfx* dlStart;
    /* 0x08 */ Gfx* dlEnd;
    /* 0x0C */ void* framebuffer; // active context's framebuffers[fadeMode] at save time
    /* 0x10 */ char unk10[0x4];
} unk_D_800A7450; // size >= 0x14

typedef struct unk_func_80007444 {
    /* 0x00 */ s16 width; // StageContext_Allocate: 0x280 (highRes) or 0x140
    /* 0x02 */ s16 height; // 0x1E0 (highRes) or 0xF0
    /* 0x04 */ s16 clipX0;
    /* 0x06 */ s16 clipX1; // width - 1
    /* 0x08 */ s16 clipY0;
    /* 0x0A */ s16 clipY1; // height - 1
    /* 0x0C */ s8 isHighRes; // StageContext_Allocate's arg0
    /* 0x0D */ s8 unk_0D; // StageContext_Allocate's arg1
    /* 0x0E */ s8 bufferCount; // StageContext_Allocate's arg2; loop bound for allocating framebuffers[]
    /* 0x0F */ s8 hasDepthBuffer; // StageContext_Allocate's arg3; ==1 allocates+attaches a shared depth buffer
    /* 0x10 */ s8 unk_10; // StageContext_Allocate's arg4
    /* 0x11 */ u8 isActive;
    /* 0x12 */ u8 fadeDuration; // StageFade_Start's arg0
    /* 0x13 */ u8 fadeTimer; // counts 0..fadeDuration
    /* 0x14 */ u16 clearColor; // Display_GetFramebufferClearColor at allocation time
    /* 0x16 */ s8 fadeMode; // StageContext_GetFadeMode
    /* 0x17 */ u8 pad17;
    /* 0x18 */ unk_D_80068BB0* framebuffers[3];
